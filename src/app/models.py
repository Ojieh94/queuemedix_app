...
from sqlalchemy import Integer, String, DateTime, Enum as pgEnum
...
class User(SQLModel, table=True):
    ...
    role: UserRoles = Field(sa_column=Column(pgEnum(UserRoles, name="user_role", create_type=True), nullable=False))
    ...
    #Relationships
    sent_messages: List["Message"] = Relationship(back_populates="sender",sa_relationship_kwargs={"foreign_keys": "[Message.sender_uid]"})
    received_messages: List["Message"] = Relationship(back_populates="receiver",sa_relationship_kwargs={"foreign_keys": "[Message.receiver_uid]"})
    hospital: Optional["Hospital"] = Relationship(back_populates="user", sa_relationship_kwargs={"lazy": "selectin"})
...
class Hospital(SQLModel, table=True):
    ...
    uid: uuid.UUID = Field(default_factory=uuid.uuid4, sa_column=Column(pg.UUID(as_uuid=True), nullable=False, primary_key=True))
    hospital_name: str = Field(default=None, sa_column=Column(String, unique=True, nullable=False))
    ...
    #Relationship
    user: "User" = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin"})
    doctor: List["Doctor"] = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin"}, passive_deletes=True)
    admin: List["Admin"] = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin"}, passive_deletes=True)
    appointment: List["Appointment"] = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin"}, passive_deletes=True)
    department: List["Department"] = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin", "cascade": "all, delete-orphan"}, passive_deletes=True)
    medical_record: List["MedicalRecord"] = Relationship(back_populates="hospital", sa_relationship_kwargs={"lazy": "selectin", "cascade": "all, delete-orphan"}, passive_deletes=True)
...
class Patient(SQLModel, table=True):
    ...
    #Relationship
    user: "User" = Relationship(back_populates="patient", sa_relationship_kwargs={"lazy": "selectin"})
    appointment: List["Appointment"] = Relationship(back_populates="patient", sa_relationship_kwargs={"lazy": "selectin"}, passive_deletes=True)
    medical_record: List["MedicalRecord"] = Relationship(back_populates="patient", sa_relationship_kwargs={"lazy": "selectin", "cascade": "all, delete-orphan"}, passive_deletes=True)
...
class Admin(SQLModel, table=True):
    ...
    #Relationships
    user: "User" = Relationship(back_populates="admin", sa_relationship_kwargs={"lazy": "selectin"})
    hospital: "Hospital" = Relationship(back_populates="admin", sa_relationship_kwargs={"lazy": "selectin"})
    department: Optional["Department"] = Relationship(back_populates="admin", sa_relationship_kwargs={"lazy": "selectin"})
...
class Department(SQLModel, table=True):
    ...
    uid: uuid.UUID = Field(default_factory=uuid.uuid4, sa_column=Column(pg.UUID(as_uuid=True), primary_key=True, index=True))
    hospital_uid: uuid.UUID = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("hospitals.uid", ondelete="CASCADE"), nullable=False, index=True, unique=True))
...
class MedicalRecord(SQLModel, table=True):
    ...
    doctor_uid: uuid.UUID = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("doctors.uid", ondelete="CASCADE"), nullable=False, index=True))
    hospital_uid: Optional[uuid.UUID] = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("hospitals.uid", ondelete="CASCADE"), nullable=True, index=True))
    record_type: RecordType = Field(default=RecordType.PRESCRIPTION, sa_column=Column(pg.ENUM(RecordType, name="record_type", create_type=True), nullable=True))
    description: str
    ...
    #Relationships
    files: List["MedicalRecordFile"] = Relationship(back_populates="medical_record", sa_relationship_kwargs={"lazy": "selectin", "cascade": "all, delete-orphan"}, passive_deletes=True)
    patient: "Patient" = Relationship(back_populates="medical_record", sa_relationship_kwargs={"lazy": "selectin"})
    doctor: "Doctor" = Relationship(back_populates="medical_record", sa_relationship_kwargs={"lazy": "selectin"})
    hospital: "Hospital" = Relationship(back_populates="medical_record", sa_relationship_kwargs={"lazy": "selectin"})
...
class MedicalRecordFile(SQLModel, table=True):
    ...
    uid: uuid.UUID = Field(default_factory=uuid.uuid4, sa_column=Column(pg.UUID(as_uuid=True), primary_key=True, index=True))
    record_uid: uuid.UUID = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("medical_records.uid", ondelete="CASCADE"), nullable=False, index=True))
    ...
    #Relationship
    medical_record: Optional[MedicalRecord] = Relationship(back_populates="files", sa_relationship_kwargs={"lazy": "selectin"})
...
class Message(SQLModel, table=True):
    ...
    uid: uuid.UUID = Field(default_factory=uuid.uuid4, sa_column=Column(pg.UUID(as_uuid=True), primary_key=True, index=True))
    sender_uid: uuid.UUID = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("users.uid", ondelete="CASCADE"), nullable=False, index=True))
    receiver_uid: uuid.UUID = Field(sa_column=Column(pg.UUID(as_uuid=True), ForeignKey("users.uid", ondelete="CASCADE"), nullable=False, index=True))
    content: str
